...
int UDPServer::initSocket() {
  ...
  emit actPrintMsg(tr("UDP: my username: %1").arg(*username));
  ...
  foreach (const QHostAddress &address, QNetworkInterface::allAddresses()) {
    if (address.protocol() == QAbstractSocket::IPv4Protocol &&
      ...
      if (count == 0) {
        emit actPrintMsg(
            tr("UDP: local IP address: %1").arg(address.toString()));
      } else {
        emit actPrintMsg(
  ...
  this->send((tr("Hi all my name is %1").arg(*username)).toStdString().c_str());
  return 0;
}
